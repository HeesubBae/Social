...
<?php
	...
	oci_bind_by_name($stmt, ':idx', $company_type_id);
...
<body>
 <div id="admin_common_include_title">
  ...
  <p class="title_text">회사유형수정</p>
  <p class="title_detail_text">회사유형정보 수정하는 페이지 입니다.</p>
 </div>
...
<form name="form" method="post" action="index.php?p=company/company_type_edit_ok"> 
 <input type=hidden name=company_type_id value="<?=$data["COMPANY_TYPE_ID"] ?>" />
 <table id="admin_common_wirte_table">
  <tr>
   <th>회사유형번호</th>
   <td>
		<input type=text class=input_size30 name=company_type_id value="<?php echo $data["COMPANY_TYPE_ID"]?>" readonly/>
   </td>
  </tr>
  <tr>
   <th>회사유형명</th>
   <td>
   <input type=text class=input_size30 name=company_type_name value="<?php echo $data["COMPANY_TYPE_NAME"]?>" />
   </td>
  </tr>  
