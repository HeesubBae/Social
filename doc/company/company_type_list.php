...
<body>
 ...
 <span class="title_icon"></span>
  <p class="title_text">회사유형정보</p>
  <p class="title_detail_text">등록된 회사유형정보 리스트페이지입니다.</p>
 </div>
 <div id="admin_common_include_subject">
 <span class="subject_icon"></span>
 <p class="subject_text">회사유형 기본정보리스트</p>
...
?>
 ...
 <form name="form" method="get" action="">
  &nbsp; 통합검색
  <?php get_company_t_list_search_type($company_t_search_type); ?>
  <input type="hidden" name="p" value="<?php echo $_GET['p'] ?>" />
  <input class="input_size10" name="company_t_search_txt" value="<?=$_GET['company_t_search_txt']?>"/> 
...
<?php
	...
	$sql = 'begin company_type_select_list(:i_txt, :i_nowpage, :i_pagesize, :v_out, :i_search_type); end;';
	...
	$company_t_search_txt=$_GET['company_t_search_txt'];

	oci_bind_by_name($stmt, ':i_txt', $company_t_search_txt);
	...
	oci_bind_by_name($stmt, ':i_search_type', $company_t_search_type);
...
?>
 ...
 <table id="admin_common_list_table">
  <tr>
   <th  style="width:5%">회사유형번호</th>
   <th  style="width:10%">회사유형명</th>
  </tr>
 <?php
	while($data=oci_fetch_array($out)){
	$TOTCNT=$data["TOT_CNT"];
?>
		<tr onClick=reback_company_type_list('<?=$data["COMPANY_TYPE_ID"]?>','<?=$data["COMPANY_TYPE_NAME"]?>')>
			<td><b><a href=index.php?p=company/company_type_view&company_type_id=<?=$data["COMPANY_TYPE_ID"]?>><?=$data["COMPANY_TYPE_ID"]?></a></b></td>
