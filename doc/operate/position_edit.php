...
<?php
	...
	$sql = 'begin position_select(:idx , :v_out); end;';
	...
	oci_bind_by_name($stmt, ':idx', $position_id);
...
<form name="form" method="post" action="index.php?p=operate/position_edit_ok"> 
 <input type=hidden name=position_id value="<?=$data["POSITION_ID"] ?>" />
 <table id="admin_common_wirte_table">
  <tr>
   <th>부서번호</th>
   <td>
		<input type=text class=input_size30 name=position_id value="<?php echo $data["POSITION_ID"]?>" readonly/>
   </td>
  </tr>
  <tr>
   <th>직책명</th>
   <td>
   <input type=text class=input_size30 name=position_name value="<?php echo $data["POSITION_NAME"]?>" />
   </td>
  </tr>  
