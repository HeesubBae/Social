...
<body>
 ...
 <span class="title_icon"></span>
  <p class="title_text">1차카테고리정보</p>
  <p class="title_detail_text">등록된 1차카테고리정보 리스트페이지입니다.</p>
 </div>
 <div id="admin_common_include_subject">
 <span class="subject_icon"></span>
 <p class="subject_text">1차카테고리 기본정보리스트</p>
 ...
 <form name="form" method="get" action="">
  &nbsp; 통합검색
  <?php get_category_code01_list_search_type($category_code01_search_type); ?>
  <input type="hidden" name="p" value="<?php echo $_GET['p'] ?>" />
  <input class="input_size10" name="category_code01_search_txt" value="<?=$_GET['category_code01_search_txt']?>"/> 
...
<?php
	...
	$sql = 'begin category_code01_select_list(:i_txt, :i_nowpage, :i_pagesize, :v_out, :i_search_type); end;';
	...
	$category_code01_search_txt=$_GET['category_code01_search_txt'];

	oci_bind_by_name($stmt, ':i_txt', $main_category_search_txt);
	...
	oci_bind_by_name($stmt, ':i_search_type', $category_code01_search_type);
...
?>
 ...
 <table id="admin_common_list_table">
  <tr>
   <th  style="width:5%">1차카테고리번호</th>
   <th  style="width:5%">1차카테고리명</th>
  </tr>
 ...
 <?php
	...
	$TOTCNT=$data["TOT_CNT"];
?>
			<tr onClick=reback_category_code01_list('<?=$data["CATEGORY_CODE01_ID"]?>','<?=$data["CATEGORY_CODE01_NAME"]?>')>
			<td><b><a href=index.php?p=display/category_code01_view&category_code01_id=<?=$data["CATEGORY_CODE01_ID"]?>><?=$data["CATEGORY_CODE01_ID"]?></a></b></td>
