...
<body>
 ...
 <span class="title_icon"></span>
  <p class="title_text">메인카테고리정보</p>
  <p class="title_detail_text">등록된 메인카테고리정보 리스트페이지입니다.</p>
 </div>
 <div id="admin_common_include_subject">
 <span class="subject_icon"></span>
 <p class="subject_text">메인카테고리 기본정보리스트</p>
 ...
 <form name="form" method="get" action="">
  &nbsp; 통합검색
  <?php get_main_category_list_search_type($main_category_search_type); ?>
  <input type="hidden" name="p" value="<?php echo $_GET['p'] ?>" />
  <input class="input_size10" name="main_category_search_txt" value="<?=$_GET['main_category_search_txt']?>"/> 
...
<?php
	...
	$sql = 'begin main_category_select_list(:i_txt, :i_nowpage, :i_pagesize, :v_out, :i_search_type); end;';
	...
	$main_category_search_txt=$_GET['main_category_search_txt'];
	...
	oci_bind_by_name($stmt, ':i_search_type', $main_category_search_type);
...
?>
 ...
 <table id="admin_common_list_table">
  <tr>
   <th  style="width:5%">메인카테고리번호</th>
   <th  style="width:5%">메인카테고리명</th>
  </tr>
 ...
 <?php
	...
	$TOTCNT=$data["TOT_CNT"];
?>
			<tr onClick=reback_main_category_list('<?=$data["MAIN_CATEGORY_ID"]?>','<?=$data["MAIN_CATEGORY_NAME"]?>')>
			<td><b><a href=index.php?p=display/main_category_view&main_category_id=<?=$data["MAIN_CATEGORY_ID"]?>><?=$data["MAIN_CATEGORY_ID"]?></a></b></td>
